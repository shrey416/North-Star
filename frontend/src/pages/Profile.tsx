...
import { Combobox } from '@/components/ui/ours/Combobox'; // Import Combobox
...
const ProfilePage = () => {
  ...
  const [studyStatus, setStudyStatus] = useState(''); // New state for current study level
    ...
    if (user) {
      const fetchProfile = async () => {
        ...
        if (docSnap.exists()) {
          ...
          setPersonalInfo({ displayName: data.displayName || '', age: data.age || '', dob: data.dob || '' });
          setStudyStatus(data.studyStatus || ''); // Fetch study status
          setEducation(data.education || []);
          setExperience(data.experience || []);
          setCareerInterests({
            interestedFields: data.interestedFields || [],
            interests: data.interests || [],
            blacklistedFields: data.blacklistedFields || [],
          });
        }
      };
      fetchProfile();
  ...
  // Updated to handle Combobox (direct value) and Input (event object)
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await setDoc(doc(db, 'users', user.uid), {
        ...
        studyStatus, // Save the new field
        education,
        experience,
        ...careerInterests,
  ...
  if (!user) return <div className="text-center p-12">Loading...</div>;
              ...
              <Card className="p-6 mt-6 space-y-6">
                ...
                <div>
                  ...
                  <Select value={studyStatus} onValueChange={setStudyStatus}>
                      ...
                      <SelectContent>
                          {STUDYING_LEVELS.map(level => <SelectItem key={level} value={level}>{level}</SelectItem>)}
                      </SelectContent>
                  </Select>
                </div>
                ...
                {education.map((entry) => (
                  <div key={entry.id} className="p-4 border rounded-md relative space-y-4">
                    <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={() => removeEducationEntry(entry.id)}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <Combobox 
                                ...
                                onChange={(value) => handleEducationChange(entry.id, 'institution', value)}
                        ...
                        <div>
                            ...
                            <Combobox 
                                ...
                                value={entry.degree}
                                onChange={(value) => handleEducationChange(entry.id, 'degree', value)}
                    ...
                    <div>
                        ...
                        <Input placeholder="e.g., Computer Science" value={entry.fieldOfStudy} onChange={(e) => handleEducationChange(entry.id, 'fieldOfStudy', e.target.value)} />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Button type="button" variant="outline" className="w-full" onClick={addEducationEntry}>
                  <PlusCircle className="mr-2 h-4 w-4" /> Add Past Education
                </Button>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="experience">
                ...
                <Card className="p-6 mt-6">
                    {experience.map(entry => (
                        <div key={entry.id} className="p-4 border rounded-md mb-4 relative">
                           <Button variant="ghost" size="icon" className="absolute top-2 right-2" onClick={() => removeExperienceEntry(entry.id)}><Trash2 className="h-4 w-4" /></Button>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div className="space-y-6">
       <div>
          ...
          <MultiSelect 
            ...
            selected={careerInterests.interests} 
            // FIX: Explicitly type 'newSelection'
            onChange={(newSelection: string[]) => 
              setCareerInterests(prev => ({ ...prev, interests: newSelection }))
            } 
          />
       </div>
       <div>
          <Label>Interested Career Fields</Label>
          <MultiSelect 
            options={careerFieldOptions} 
            selected={careerInterests.interestedFields}
            // FIX: Explicitly type 'newSelection'
            onChange={(newSelection: string[]) => 
              setCareerInterests(prev => ({ ...prev, interestedFields: newSelection }))
            }
          />
       </div>
        <div>
          <Label>Fields to Avoid</Label>
          <MultiSelect 
            options={careerFieldOptions} 
            selected={careerInterests.blacklistedFields} 
            // FIX: Explicitly type 'newSelection'
            onChange={(newSelection: string[]) => 
              setCareerInterests(prev => ({ ...prev, blacklistedFields: newSelection }))
            }
          />
       </div>
    </div>
